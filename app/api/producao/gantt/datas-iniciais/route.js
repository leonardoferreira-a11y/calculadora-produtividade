...
export async function GET(request) {
  try {
    ...
    const res = await pool.query(
      `SELECT dt_calculo FROM prod_datas_iniciais
    ...
    return NextResponse.json({ dt_calculo: res.rows[0]?.dt_calculo || null });
  } catch (e) {
...
export async function POST(request) {
  try {
    const { sku, filtro_producao, grafica, dt_calculo } = await request.json();
    if (!sku || !filtro_producao || !grafica) {
      return NextResponse.json({ message: "sku, filtro_producao e grafica são obrigatórios." }, { status: 400 });
    }
    await pool.query(
      `INSERT INTO prod_datas_iniciais (sku, filtro_producao, grafica, dt_calculo)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (sku, filtro_producao, grafica)
       DO UPDATE SET dt_calculo = EXCLUDED.dt_calculo`,
      [sku.toUpperCase().trim(), filtro_producao.toUpperCase().trim(), grafica.toUpperCase().trim(), dt_calculo]
    );
