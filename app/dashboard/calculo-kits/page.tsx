  ...
  const carregarKitsDoLote = async (loteEscolhido: string, graficaEscolhida: string) => {
    ...
    try {
      ...
      if (res.ok && dados.length > 0) {
        // prod_kits guarda 1 linha por componente (capa/SKU) do kit, então o
        // mesmo id_codigo_kit vem repetido várias vezes. A Fila de Montagem
        // trabalha por KIT (1 pacote), não por componente. Deduplicamos por
        // id_codigo_kit (preferindo a linha que já tem cálculo) para evitar
        // linhas/keys repetidas e totais de tiragem/tempo inflados.
        const porKit = new Map<string, any>();
        for (const linha of dados) {
          const chave = String(linha.id_codigo_kit || '').trim().toUpperCase();
          const existente = porKit.get(chave);
          if (!existente || (!existente.dados_calculo && linha.dados_calculo)) {
            porKit.set(chave, linha);
          }
        }
        setKits(Array.from(porKit.values()));
        setEtapa(2);
      } else {
    ...
    <div className="w-full text-left font-sans relative">
      ...
      {feedback.msg && !modalRobo && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white border-b-4 border-indigo-600 rounded-2xl shadow-2xl p-8 flex flex-col items-center max-w-sm">
      ...
      {modalRobo && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[400px] p-8 text-center flex flex-col items-center">
      ...
      {/* 🧹 MODAL DA BORRACHA */}
      {modalConfigLimpar && (
        <div className="fixed inset-0 z-[9500] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            ...
            <div className="p-6">
              ...
              <div className="space-y-4 mb-6 relative">
                <div className="relative z-[9600]">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setDropdownLimpar(dropdownLimpar === 'tipo_kit' ? null : 'tipo_kit'); }} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 text-xs font-bold text-slate-700 text-left flex justify-between items-center shadow-sm">
                ...
                <div>
                  ...
                  <input type="text" placeholder="Digite para apagar um kit exato..." value={limparFiltros.sku} onChange={(e) => setLimparFiltros({...limparFiltros, sku: e.target.value})} className="w-full border border-slate-200 rounded-lg p-2.5 text-xs font-mono font-bold text-slate-800 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" />
                </div>
              </div>
        ...
        return (
          <>
            <header className="mb-6 border-b border-slate-200 pb-4 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
              <div>
                <h1 className="text-2xl font-black text-slate-900 tracking-tight">Módulo de Montagem de Kits</h1>
                <p className="text-sm text-slate-500 font-medium mt-1">Gerenciamento de empacotamento, shrinkagem de segurança e fator de complexidade.</p>
              </div>
              ...
              <select
                ...
                className="w-full md:w-auto border border-slate-200 rounded-lg p-2.5 text-sm font-bold text-slate-700 bg-white shadow-sm outline-none cursor-pointer focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
              >
              ...
              <table className="w-full text-left border-collapse text-sm min-w-[640px]">
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase text-[10px] font-bold tracking-wider">
                  <tr>
                    ...
                    <th className="p-3 text-center border-l border-slate-200 w-24">Total Projetos</th>
                    ...
                    return (
                      <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="p-3 text-right pr-4">
                          <button onClick={() => carregarKitsDoLote(l.filtro_producao, l.grafica)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded-lg text-xs font-bold shadow-sm shadow-blue-600/20 transition-all">
                            Abrir Fila
                          </button>
                        </td>
                      </tr>
        ...
        return (
          <>
            <header className="mb-4 border-b border-slate-200 pb-4 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
              <div><h1 className="text-2xl font-black text-slate-900 tracking-tight">Fila de Montagem de Kits</h1><p className="text-sm text-slate-500 mt-1 font-mono">Ref: {filtroProducao} / {grafica}</p></div>
              <div className="flex flex-col sm:flex-row gap-2">
                {kitsConcluidosLote > 0 && (
                  <button onClick={() => { setLimparFiltros({ tipo_kit: [], sku: '' }); setModalConfigLimpar(true); }} className="text-sm text-red-600 font-bold border border-red-200 px-4 py-2 rounded-lg hover:bg-red-50 shadow-sm transition-all">
                    <i className="fas fa-eraser mr-1"></i> Borracha de Engenharia
                  </button>
                )}
                <button onClick={() => { setEtapa(1); carregarLotesIniciais(); }} className="text-sm text-indigo-600 font-bold border border-indigo-200 px-4 py-2 rounded-lg hover:bg-indigo-50 shadow-sm transition-all">
                  Voltar aos Lotes
                </button>
              </div>
            </header>
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-3 mb-5 p-3 bg-slate-50 border border-slate-200 rounded-xl shadow-sm">
              <i className="fas fa-tools text-slate-400 text-sm hidden md:inline"></i>
              <select
                ...
                className="w-full md:w-auto border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-slate-700 bg-white outline-none cursor-pointer md:min-w-[220px] focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
              >
                <option value="">Selecionar Kit específico...</option>
                {kits.map((k: any) => (
                  <option key={k.id ?? k.id_codigo_kit} value={k.id_codigo_kit}>
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5 border border-amber-300 text-amber-700 bg-amber-50 hover:bg-amber-100 font-bold px-3 py-2 rounded-lg text-sm transition-all"
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5 border border-emerald-300 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 font-bold px-3 py-2 rounded-lg text-sm transition-all"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm border-l-4 border-l-indigo-600">
              ...
              <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm border-l-4 border-l-emerald-600 md:col-span-2">
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-col md:flex-row gap-4 mb-4 pb-4 border-b border-indigo-200 relative z-[8100]">
                    <div className="flex-1 relative">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); setDropdownRobo(dropdownRobo === 'tipo_kit' ? null : 'tipo_kit'); }} className="w-full bg-white border border-indigo-300 rounded-lg p-2.5 text-xs font-bold text-slate-700 text-left flex justify-between items-center shadow-sm">
                    ...
                    <div className="flex-1 relative">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); setDropdownRobo(dropdownRobo === 'shrink' ? null : 'shrink'); }} className="w-full bg-white border border-indigo-300 rounded-lg p-2.5 text-xs font-bold text-slate-700 text-left flex justify-between items-center shadow-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                    <div>
                      ...
                      <select value={robo.encaixotamento} onChange={e => setRobo({...robo, encaixotamento: e.target.value})} className="w-full border border-indigo-300 rounded-lg p-2.5 text-xs font-bold text-slate-700 bg-white outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                    ...
                    <div>
                      ...
                      <select value={robo.shrink} onChange={e => setRobo({...robo, shrink: e.target.value})} className="w-full border border-indigo-300 rounded-lg p-2.5 text-xs font-bold text-slate-700 bg-white outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all">
                        <option value="">Automático...</option>
                  ...
                  <div className="flex justify-end">
                    <button onClick={executarAutomacaoKits} disabled={skusAlvoRobo.length === 0} className="w-full sm:w-auto justify-center bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-400 text-white font-bold px-6 py-2.5 rounded-lg shadow-sm shadow-indigo-600/20 flex items-center gap-2 text-xs uppercase tracking-wide transition-all">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
              <table className="w-full text-left border-collapse text-sm whitespace-nowrap min-w-[900px]">
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase text-[10px] font-bold tracking-wider">
                  <tr>
                    ...
                    <th className="p-3 text-right w-24 border-r border-slate-200">Tiragem</th>
                    <th className="p-3 text-center w-24 text-blue-800 bg-blue-50/50">T. Embalar</th>
                    <th className="p-3 text-center w-24 border-r border-slate-200 text-purple-800 bg-purple-50/50">T. Shrink</th>
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                  {kits.map((k, idx) => {
                    ...
                    const tBox = temCalculo && src.encaixotamento?.resultado ? (src.encaixotamento.resultado.totais?.total ?? 'Pend.') : 'Pend.';
                    ...
                    return (
                      <tr key={idx} className="hover:bg-slate-50/50 transition-colors cursor-pointer group" onClick={() => {
                        setKitAtual(k);
                        ...
                        <td className="p-3 text-right font-mono font-bold text-slate-700 border-r border-slate-200">{Number(k.tiragem).toLocaleString('pt-BR')}</td>
                        ...
                        <td className={`p-3 text-center font-mono text-xs border-r border-slate-200 ${tShrink === '-' ? 'text-slate-400 italic' : 'font-bold text-purple-900 bg-purple-50/20'}`}>{tShrink}</td>
      ...
      {etapa === 3 && kitAtual && (() => {
        ...
        return (
          <div className="w-full text-left font-sans text-slate-800 pb-24">
            <div className="bg-slate-800 text-white rounded-t-xl p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 shadow-sm">
              <div>
              ...
              <button onClick={() => setEtapa(2)} className="bg-slate-700 hover:bg-slate-600 text-xs font-bold px-4 py-2 rounded-lg shadow-sm transition-all">Voltar para a Fila</button>
            </div>

            <div className="bg-white border-x border-b border-slate-200 rounded-b-xl p-5 grid grid-cols-2 md:grid-cols-4 gap-6 shadow-sm mb-6">
            ...
            <div className="bg-white border border-slate-200 rounded-lg shadow-sm mb-4 overflow-hidden border-l-4 border-l-blue-600">
              <div className="bg-slate-50 border-b border-slate-200 p-3 flex justify-between items-center cursor-pointer select-none" onClick={() => toggleSecao('encaixotamento')}>
                <h3 className="text-sm font-bold uppercase text-slate-800 flex items-center gap-2"><span className="bg-blue-600 text-white w-6 h-6 flex items-center justify-center rounded-lg text-xs">1</span> Estação de Encaixotamento Manual / Auto</h3>
              ...
              {secoes.encaixotamento && (
                <div className="p-4">
                  <select value={idMaquinaEncaixotamento} onChange={e => setIdMaquinaEncaixotamento(e.target.value)} className="w-full border border-slate-200 rounded-lg p-2.5 text-sm font-bold text-slate-700 bg-white mb-3 outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                  ...
                  {analiseBox && (
                    <div className="mt-4 border border-slate-200 rounded-lg overflow-x-auto">
                      <div className="bg-slate-50 border-b border-slate-200 p-2 flex flex-wrap gap-x-6 gap-y-1 text-xs font-bold text-slate-600 uppercase tracking-wide">
                      ...
                      <table className="w-full text-left border-collapse text-sm bg-white min-w-[560px]">
                        <thead className="bg-slate-100 border-b border-slate-200 text-[10px] uppercase font-bold text-slate-500 tracking-wider">
            ...
            {String(kitAtual.com_shrink || '').toUpperCase() === 'SIM' && (
              <div className="bg-white border border-slate-200 rounded-lg shadow-sm mb-4 overflow-hidden border-l-4 border-l-purple-600">
                <div className="bg-purple-50/30 border-b border-slate-200 p-3 flex justify-between items-center cursor-pointer select-none" onClick={() => toggleSecao('shrink')}>
                  <h3 className="text-sm font-bold uppercase text-purple-950 flex items-center gap-2"><span className="bg-purple-600 text-white w-6 h-6 flex items-center justify-center rounded-lg text-xs">2</span> Seladora Termoencolhível (Túnel Shrink)</h3>
                  <div className="flex items-center gap-4 relative">
                ...
                {secoes.shrink && (
                  <div className="p-4">
                    <select value={idMaquinaShrink} onChange={e => setIdMaquinaShrink(e.target.value)} className="w-full border border-slate-200 rounded-lg p-2.5 text-sm font-bold text-slate-700 bg-white mb-3 outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all">
                    ...
                    {analiseShrink && (
                      <div className="mt-4 border border-slate-200 rounded-lg overflow-x-auto">
                        <div className="bg-purple-50/50 border-b border-slate-200 p-2 flex flex-wrap gap-x-6 gap-y-1 text-xs font-bold text-purple-900 uppercase tracking-wide">
            ...
            {analiseBox && (
              <div className="flex justify-end mt-6">
                <button onClick={() => salvarKitNoBanco(analiseBox, analiseShrink)} className="w-full sm:w-auto justify-center bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3 rounded-lg shadow-sm shadow-blue-600/20 flex items-center gap-2 text-sm uppercase tracking-wide transition-all">
